...
function receive_command(command) {
  winston.info('Command Processer -  CMD: ' + command.cmdCode);
  winston.debug('Command Processer -  CMD:', command)

  // 요청커맨드로 처리되는 방식과 받은 Command로 처리되는 방식으로 나눈다.
  if (command.sendCmd) {

    winston.info('IS DS_BASE:' , parseInt(command.cmdCode/CommandCodes.DS_BASE))
    ...
    if (parseInt(command.cmdCode/CommandCodes.DS_BASE) == 1) {
      ...
      winston.warn('Can not find Command Group! -  CMD: ' + command.cmdCode);
    }
  } else {
    //
    // 요청없이 받은 데이터
    ...
    switch( command.cmdCode ) {
      ...
      default :
        ...
        winston.warn('Unknown Command Receive: ' + command.cmdCode); // + ' Data:' + dataStr);
