...
function responseCmdProc(recvCmd) {
  if (!recvCmd.sendCmd) {
    sendLog('FETCH Request Command Empty! -  CMD: ' + recvCmd.cmdCode);
    return;
  }

  sendLog('FETCH Response -  RES_CMD: ' + recvCmd.cmdCode);
  console.log('RESCMD-', recvCmd)
  ...
  switch(recvCmd.sendCmd.cmdCode) {
    case CmdCodes.FETCH_SQL_REQUEST:
      if (recvCmd.cmdCode == CmdCodes.FETCH_SELECT_SUCCESS) {
        ...
        parseXmlToJSON(result).then(function(jsonData) {
          ...
          console.log('FETCH_SQL_REQUEST  xml parse Error! str:', result, err)
    ...
    default :
    {
      ...
      sendLog('Unknown Response Command Receive!!! : ' + recvCmd.cmdCode + ' Data:' + dataStr);
    }
    return false;
  }
