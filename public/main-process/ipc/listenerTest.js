...
ipcMain.on('testAction', async (event, ...args) => {
  ...
  logger.info('BUF_ENC', testBuf)
  let encBuf = CryptoUtil.encryptBufferRC4(encKey, testBuf);
  logger.info('BUF_ENC ENCRYPTO', encBuf)
  let decBuf = CryptoUtil.decryptBufferRC4(encKey, encBuf);
  logger.info('BUF_ENC DECRYPTO', decBuf)
  logger.info('BUF_ENC', decBuf.toString('utf8'));
  ...
  */
  ...
  logger.info('DATE>>', OsUtil.getDateString(DATE_FORMAT.YYYYMMDDHHmmssSSS));
  ...
  // logger.info('Random Pwd 4', pwd4.length, pwd4);

  // let pwd32 = CryptoUtil.randomPassword(32);
  // logger.info('Random Pwd 32', pwd32.length, pwd32);
  ...
  logger.info('ENC: ' + encTxt);

  let decTxt = CryptoUtil.decryptRC4(key, encTxt);
  logger.info('DES: ' + decTxt);
  ...
  */
  ...
  logger.info('testFunction res:', resData)
...
ipcMain.on('connectDS', async (event, ...args) => {
  ...
  reqConnectDS().then(function (resData) {
    logger.info('DS CONNECTIN SUCCESS! res:', resData)
    event.reply('res-connectDS', resData);
  }).catch(function (err) {
    logger.info('DS CONNECTIN FAIL!', err)
...
ipcMain.on('upgradeCheck', async (event, ...args) => {
  ...
  reqUpgradeCheckDS(function (resData) {
    logger.info('upgradeCheck res:', resData)
