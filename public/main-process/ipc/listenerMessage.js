...
ipcMain.on('sendMessage', async (event, recvIds, recvNames, subject, message, attFileInfo) => {
  logger.info('[IPC] sendMessage ', {recvIds:recvIds, recvNames:recvNames, subject:subject, message:message, attFileInfo:attFileInfo})
  
  
  nsAPI.reqSendMessage(recvIds, recvNames, subject, message, attFileInfo).then(function (resData) {
    logger.info('[IPC] sendMessage res:', resData)
    event.reply('res-sendMessage', resData);
  }).catch(function (err) {
    logger.info('[IPC] sendMessage res  Err:', err)
...
ipcMain.on('getMessage', async (event, msgType, rowOffset = 0, rowLimit = 100) => {
  ...
  fetchAPI.reqMessageList(msgType, rowOffset, rowLimit).then(function (resData) {
    logger.info('[IPC] getMessage res:', resData)
    event.reply('res-getMessage', resData);
  }).catch(function (err) {
    logger.info('[IPC] getMessage res  Err:', err)
...
ipcMain.on('getMessageDetail', async (event, msgKey) => {
  ...
  fetchAPI.reqGetMessageDetail(msgKey).then(function (resData) {
    logger.info('[IPC] getMessageDetail res:', resData)
    event.reply('res-getMessageDetail', resData);
  }).catch(function (err) {
    logger.info('[IPC] getMessageDetail res  Err:', err)
...
ipcMain.on('deleteMessage', async (event, msgType, msgKeys) => {
  ...
  try {
    ...
    logger.info('[IPC] deleteMessage res:', resData)
    event.reply('res-deleteMessage', resData);
  } catch (err) {
    logger.info('[IPC] deleteMessage res  Err:', err)
...
ipcMain.on('getChatRoomList', async (event, msgKey) => {
  ...
  fetchAPI.reqChatRoomList(msgKey).then(function (resData) {
    logger.info('[IPC] getChatRoomList res:', resData)
    event.reply('res-getChatRoomList', resData);
  }).catch(function (err) {
    logger.info('[IPC] getChatRoomList res  Err:', err)
...
ipcMain.on('getChatRoomByRoomKey', async (event, roomKey) => {
  ...
  fetchAPI.reqChatRoomByRoomKey(roomKey).then(function (resData) {
    logger.info('[IPC] getChatRoomByRoomKey res:', resData)
    event.reply('res-getChatRoomByRoomKey', resData);
  }).catch(function (err) {
    logger.info('[IPC] getChatRoomByRoomKey res  Err:', roomKey, err)
...
ipcMain.on('sendChatMessage', async (event, chatUserIds, chatMessage, chatFontName, roomKey = null, roomTitle = '', type) => {
  ...
  logger.info('[IPC] sendChatMessage :', roomKey, chatUserIds, chatMessage, type);

  // RoomKey가 없다면 만든다.
  if (!roomKey) {
    // 1:1이라면
    roomKey = createChatRoomKey(chatUserIds);
    logger.info('[IPC] createChatRoomKey :', roomKey);
  }

  nsAPI.reqChatLineKey(roomKey).then(function (resData) {
    logger.info('[IPC] getChatLineKey res:', resData)
    ...
    if (resData.resCode) {
      nsAPI.reqSendChatMessage(roomKey, resData.data.lineKey, chatUserIds, chatMessage, chatFontName, roomTitle, type).then(function (resData) {
        logger.info('[IPC] sendChatMessage res:', resData)
        event.reply('res-sendChatMessage', resData);

      }).catch(function (err) {
        logger.info('[IPC] sendChatMessage res  Err:', err)
        event.reply('res-sendChatMessage', new ResData(false, err));
      });
    } else {
      logger.info('[IPC] getChatLineKey res  Err:', resData)
      event.reply('res-getChatLineKey', resData);
    }


  }).catch(function (err) {
    logger.info('[IPC] getChatLineKey res  Err:', err)
...
ipcMain.on('getChatList', async (event, roomId, lastLineKey, rowLimit) => {
  ...
  fetchAPI.reqGetChatList(roomId, lastLineKey, rowLimit).then(function (resData) {
    logger.info('[IPC] getChatList res:', resData)
    event.reply('res-getChatList', resData);
  }).catch(function (err) {
    logger.info('[IPC] getChatList res  Err:', err)
...
ipcMain.on('changeChatRoomName', async (event, roomId, chatEntryNames, chatUserIds) => {
  ...
  fetchAPI.reqChangeChatRoomName(roomId, chatEntryNames).then((resData) => {
    ...
    nsAPI.reqChangeChatRoomName(roomId, chatEntryNames, chatUserIds).then(function (resData) {
      logger.info('[IPC] changeChatRoomName res:', resData)
      event.reply('res-changeChatRoomName', resData);
    }).catch(function (err) {
      logger.info('[IPC] changeChatRoomName res  Err:', err)
  ...
  logger.info('[IPC] changeChatRoomName sucess!', res)
...
ipcMain.on('inviteChatUser', async (event, roomKey, newRoomName, asIsUserIds, newUserIds) => {
  logger.debug('inviteChatUser', {roomKey:roomKey, newRoomName:newRoomName, asIsUserIds:asIsUserIds, newUserIds:newUserIds})

  nsAPI.reqInviteChatUser(roomKey, newRoomName, asIsUserIds, newUserIds).then(function (resData) {
    logger.info('[IPC] inviteChatUser res:', resData)
    event.reply('res-inviteChatUser', resData);
  }).catch(function (err) {
    logger.info('[IPC] inviteChatUser res  Err:', err)
