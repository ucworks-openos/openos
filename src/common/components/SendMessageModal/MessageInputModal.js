...
function MessageInputModal(props) {
  ...
  useEffect(() => {
    const initiate = async () => {
      ...
      const {
        data: {
          items: { node_item: responseMaybeArr },
        },
      } = await getUserInfos(props.selectedNode);
  ...
  const onSubmit = async (event) => {
    ...
    if (hasAttachmentFiles) {
      ...
      for (let i = 0; i < attachmentFiles.length; i++) {
        let resData = await uploadFile(
          attachmentFiles[i].path,
          attachmentFiles[i].path
        );
        fileUploadProgress(attachmentFiles[i].path, "100%", resData.data);

        attFileInfo += `${remote.getGlobal("SERVER_INFO").FS.pubip};${
          remote.getGlobal("SERVER_INFO").FS.port
        }|${attachmentFiles[i].name}|${attachmentFiles[i].size}|${
          resData.data
        }|`;
  ...
  return (
    <div>
      ...
      {isUserSelected && (
        <Alert variant="danger">받는 사람을 선택해주세요.</Alert>
      )}
      {isAlreadyCheckedUser && (
        <Alert variant="danger">이미 선택된 사용자 입니다.</Alert>
      )}
      {isNoUser && <Alert variant="danger">검색된 사용자가 없습니다.</Alert>}
      {selectedUsers.length > 0 && (
        <div className="write-row to-ppl-added">{renderCheckedMember()}</div>
      )}
      ...
      {isContentTyped && (
        <Alert variant="danger">쪽지 내용을 입력해 주세요.</Alert>
      )}
      <br />
      ...
      {sendBtnEnable ? (
        <div class="modal-btn-wrap" style={{ marginTop: `10px` }}>
