...
function UploadAttachmentFile(prop) {
  ...
  const removeAttachFile = (filePath) => {
    ...
    prop.setAttachmentFiles(
      prop.attachmentFiles.filter((file) => file.path !== filePath)
    );
  };
        ...
        {prop.attachmentFiles.map((file, index) => {
          return (
            <div class="attatched-file-row" key={index}>
              ...
              <div class="label-attatched-file-name">
              ...
              <div
                class="btn-attatched-file-name-remove"
                onClick={() => {
                  removeAttachFile(file.path);
                }}
              >
                &nbsp;삭제
              </div>
