...
import { addMessage } from "../../../redux/actions/message_actions";
...
function MessageInputModal(props) {
  ...
  useEffect(() => {
    const initiate = async () => {
      const {
        ...
        props.selectedUser &&
        (await searchUsers(searchMode, props.selectedUser));
    ...
    if (result.data.root_node !== "") {
      isAlreadySelectedUser =
        selectedUsers.filter(
          (user) =>
            user.user_id.value === result.data.root_node.node_item.user_id.value
        ).length !== 0;
  ...
  const onSubmit = (event) => {
    ...
    if (title.trim().length === 0) {
      return alert("먼저 쪽지 이름을 입력해 주세요.");
    }
    if (content.trim().length === 0) {
      return alert("먼저 쪽지를 입력해 주세요.");
    }
