...
function MessageFiles(prop) {
  ...
  useEffect(() => {
    ...
    electron.ipcRenderer.on(
      ...
      (event, svrName, downloadLength, fileLength) => {
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(
      "sessionDnAttFiles",
      JSON.stringify(prop.attachmentFiles)
    );
  ...
  const saveAll = async () => {
    for (let i = 0; i < prop.attachmentFiles.length; i++) {
      ...
      let resData = await downloadFile(
        prop.attachmentFiles[i].serverIp,
        prop.attachmentFiles[i].serverPort,
        prop.attachmentFiles[i].svrName,
        fullPath
      );
      writeInfo(
        "file Download completed!",
        prop.attachmentFiles[i].name,
        resData
      );
  ...
  const downloadAttFile = (svrName) => {
    let attFile = prop.attachmentFiles.filter(
      (file) => file.svrName === svrName
    );
